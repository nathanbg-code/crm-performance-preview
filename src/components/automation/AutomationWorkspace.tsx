...
export const AutomationWorkspace: React.FC<AutomationWorkspaceProps> = ({
  ...
  const [workspaceMode, setWorkspaceMode] = useState<'canvas' | 'funnel_rules'>('canvas');
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-zinc-950 select-none">
      ...
      {/* Subheader: Canvas vs Automações do Funil Mode Toggle */}
      <div className="h-10 border-b border-zinc-800 bg-zinc-900/60 px-4 flex items-center justify-between gap-3 shrink-0">
        <div className="flex items-center gap-1.5 text-xs">
          ...
          >
            <span>Fluxos Visuais (Canvas)</span>
          </button>

          <button
            onClick={() => setWorkspaceMode('funnel_rules')}
            className={`px-3 py-1 rounded-md font-medium transition cursor-pointer flex items-center gap-1.5 ${
              workspaceMode === 'funnel_rules'
                ? 'bg-v4-primary text-white shadow-xs font-semibold'
                : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-850'
            }`}
          >
            <Zap className="w-3.5 h-3.5 text-amber-400" />
            <span>Automações das Etapas do Funil</span>
          </button>
        </div>
        ...
        {onOpenFunnelBuilder && (
          <button
            ...
            className="text-xs text-v4-primary hover:text-red-400 flex items-center gap-1.5 font-medium transition cursor-pointer"
          >
            <Sliders className="w-3.5 h-3.5" />
            <span>Abrir Construtor de Funis & SLAs</span>
          </button>
        )}
      </div>

      {/* ========================================================================= */}
      {/* Workspace Body: Canvas OR Funnel Stage Rules                               */}
      {/* ========================================================================= */}
      {workspaceMode === 'canvas' ? (
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-zinc-950 space-y-6">
          <div className="max-w-5xl mx-auto space-y-5">
            {/* Header info card */}
            <div className="p-5 rounded-2xl bg-zinc-900/60 border border-zinc-800/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-zinc-100">
                    Regras de Automação por Etapa do Funil
                  </h3>
                ...
                <p className="text-xs text-zinc-400 mt-1">
                  Disparos automáticos executados quando um lead entra ou permanece ocioso em cada etapa comercial de {pipeline?.name || 'Funil Principal'}.
                </p>
              </div>
              ...
              {onOpenFunnelBuilder && (
                <button
                  onClick={onOpenFunnelBuilder}
                  className="px-3.5 py-2 rounded-xl bg-v4-primary hover:bg-v4-primary-hover text-white text-xs font-semibold shadow-md transition cursor-pointer flex items-center gap-2 shrink-0"
                >
                  <Sliders className="w-4 h-4" />
                  <span>Gerenciar Etapas & Regras</span>
                </button>
              )}
            </div>

            {/* Stages List with their automations */}
            <div className="space-y-4">
              {(pipeline?.stages || []).map((stage, sIdx) => {
                ...
                return (
                  <div
                    key={stage.id}
                    className="p-5 rounded-2xl bg-zinc-900/40 border border-zinc-800/50 space-y-4"
                  >
                    {/* Stage Header */}
                    <div className="flex items-center justify-between border-b border-zinc-800/40 pb-3">
                      <div className="flex items-center gap-3">
                        <span className="w-6 h-6 rounded bg-zinc-800 text-zinc-400 font-mono text-xs font-bold flex items-center justify-center">
                          {sIdx + 1}
                        </span>
                        <div
                        ...
                        />
                        <div>
                          <h4 className="text-xs font-semibold text-zinc-100">{stage.title}</h4>
                          <span className="text-[10px] text-zinc-500 font-mono">
                            {stage.slaMinutes && stage.slaMinutes > 0 ? `SLA: ${stage.slaMinutes} min` : 'Sem SLA'} • {rules.length} {rules.length === 1 ? 'automação' : 'automações'}
                          </span>
                        </div>
                      </div>
                      ...
                      {onOpenFunnelBuilder && (
                        <button
                          onClick={onOpenFunnelBuilder}
                          className="text-xs text-zinc-400 hover:text-zinc-100 flex items-center gap-1 transition cursor-pointer"
                        >
                    ...
                    <div className="space-y-2.5">
                      {rules.map((rule) => (
                        <div
                          key={rule.id}
                          className="p-3.5 rounded-xl bg-zinc-950/80 border border-zinc-800/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs"
                        >
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-zinc-400">
                              <span className="text-purple-400">⚡ {rule.triggerLabel}</span>
                              <span>→</span>
                              <span className="text-emerald-400">✓ {rule.actionLabel}</span>
                            </div>
                          </div>

                          <div className="flex items-center gap-3 shrink-0">
                            <span className="text-[10px] font-mono text-zinc-500">
                              {rule.executedCount} disparos
                            </span>
                            {onOpenFunnelBuilder && (
                              <button
                                onClick={onOpenFunnelBuilder}
                                className="px-2.5 py-1 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-[11px] text-zinc-300 transition cursor-pointer"
                              >
                                Editar
                              </button>
                            )}
                      ...
                      {rules.length === 0 && (
                        <div className="p-4 rounded-xl border border-dashed border-zinc-800/60 text-center text-xs text-zinc-500">
                          Nenhuma regra configurada nesta etapa.
                        </div>
                      )}
                    </div>
                  </div>
